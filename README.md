...
| !\[](docs/images/base\_config.png) | !\[](docs/images/base\_result.png) |
...
| !\[](docs/images/adain\_config.png) | !\[](docs/images/adain\_result.png) |
...
!\[](docs/images/object\_detection\_comparison.png)
...
└── weights/          # not tracked in Git, download from Releases
...
&#x20;   --neutral\_template white \\

&#x20;   --target\_path outputs \\

&#x20;   --model base\_model \\

&#x20;   --weight\_path weights/base\_model/model\_1000001.ckpt
...
&#x20;   --weight\_path weights/adain\_model/model\_1000001.ckpt
...
| `--neutral\_template` | Neutral template used for weather neutralization. Available options: `white`, `black`, `chequered`.                    |

| `--target\_path`      | Output directory where generated images will be saved. The directory is created automatically if it does not exist.    |

| `--model`            | Model architecture to use. Available options: `base\_model` and `adain\_model`.                                          |

| `--weight\_path`      | Path to the pretrained model checkpoint (`.ckpt`).                                                                     |

| `--batch\_size`       | Number of images processed simultaneously. Default: `1`.                                                               |
...
| `--keep\_resolution`  | Preserve the original image resolution instead of resizing to `height × width`.                                        |

| `--device`           | Computation device. Available options: `auto`, `cpu`, `cuda`. Default: `auto`.                                         |

| `--hot\_vector`       | Domain vector used by the model. Can be `"ones"`, `"zeros"`, or a custom comma-separated vector such as `"0,1,0,0,0"`. |
...
&#x20;   --root data/images/img\_0001.png \\
